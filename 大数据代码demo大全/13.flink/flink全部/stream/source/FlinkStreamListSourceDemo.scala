...
object FlinkStreamListSourceDemo {
  def main(args: Array[String]): Unit = {
    //创建流处理的执行环境
    val senv: StreamExecutionEnvironment = StreamExecutionEnvironment.getExecutionEnvironment
    //使用env.fromElements()来创建函数
    ...
    val ds13: DataStream[String] = senv.fromCollection(ArraySeq("spark", "flink"))
    ds13.print()
    //14.用ArrayStack创建DataStream
    val ds14: DataStream[String] = senv.fromCollection(ArrayStack("spark", "flink"))
    ds14.print()
